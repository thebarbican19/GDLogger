...
#define LOGGER_APP [[[NSBundle mainBundle] localizedInfoDictionary] objectForKey:@"CFBundleDisplayName"]
...
-(void)log:(NSString *)title properties:(NSDictionary *)properties {
    ...
    for (int i = 0; i < [[properties allKeys] count]; i++) {
        if ([[properties objectForKey:[[properties allKeys] objectAtIndex:i]] isKindOfClass:[NSNumber class]]) NSAssert(1, @"properties dictionary cannot contain NSNumber objects.");
        else [appendContents appendString:[NSString stringWithFormat:@"%@: \"%@\"\n" ,[[properties allKeys] objectAtIndex:i], [properties objectForKey:[[properties allKeys] objectAtIndex:i]]]];
